<template>
  <form @submit.prevent="confirm">
    <div class="modal task">
      <header class="title">
        <input type="text" :class="{ error: errors['title'] }" :placeholder="$t('plans.title', 'Inserisci qui il titotlo')" class="transparent title" v-model="task.title" maxlength="110" required v-validate="(errs, a) => setError('title', errs)" />
      ...
      <header class="status">
        <div class="area">
          ...
          <select v-model="task.groupId" required>
        ...
        <div class="area">
          ...
          <input type="file" :accept="imageContentTypes" @change="$event => onChangeCoverImage($event)" required />
        </div>
      </header>
      <header>
        <div class="editor">
      ...
      <header class="dates">
        <div class="area fixed">
          ...
          <div class="date-picker-container">
            <date-picker-vue v-model="task.startDate" mode="dateTime" timezone="utc" required>
